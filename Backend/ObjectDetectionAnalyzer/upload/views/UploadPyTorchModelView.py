...
class UploadPyTorchModelView(UploadBaseView):
    ...
    def save_data(self, tmp_file_path, target_dir, dataset_name, model_name, dataset, user, file_name):
        ...
        model = Models.objects.filter(name=model_name, type=ModelTypes.PYTORCH.value, userId=user)
        if model:
            model.update(uploaded_at=timezone.now())
        else:
            Models.objects.create(name=model_name, path=model_path, type=ModelTypes.PYTORCH.value, userId=user)
