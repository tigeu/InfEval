...
class TestTensorFlow2ModelView(APITestCase):
    """
    Test UploadTensorFlow2ModelView
    """

    def setUp(self):
        self.view = UploadTensorFlow2ModelView()
    ...
    def test_save_data_with_prediction(self, save_compressed_model, update):
        ...
        Models.objects.create(name="model_name", path="target", type='tf2', userId=user)
    ...
    def test_save_data_without_prediction(self, save_compressed_model, create):
        save_compressed_model.return_value = Path("new_model_name/new_model_name")
        user = User.objects.create_user("test", "test@test.test", "test")

        self.view.save_data(Path("tmp"), Path("new_model_name"), None, "new_model_name", None, None, user, "model")

        save_compressed_model.assert_called_with(Path("tmp"), Path("new_model_name"), "new_model_name")
        create.assert_called_with(name="new_model_name",
                                  path=Path("new_model_name/new_model_name"), type='tf2', userId=user)
